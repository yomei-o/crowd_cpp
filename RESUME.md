...
3. **C++ 側にも FIDT 学習と F1 評価を入れる**（Python には入れた）。`crowd train --fidt` と
   `crowd val --loc` 相当。`den::peaks` と `den::match_points` は既にあるので配線だけ。
...
- [~] **M7 FIDTM** — ラベル・デコーダ（`--decoder 2|4`）・極大検出・F1 指標・Python の学習は完了。
      C++ の学習配線と、実際の F1 測定が残り。
...
1. **入力サイズとメモリ**: 丸ごと1枚（768x1024）の学習は T4 で 0.485 秒/step、3.1GB。CPU の自作
   エンジンでは 1 step が何秒かかるか未測定（小さい合成画像でしか動かしていない）。
