...
//
//   node wasm/test_node.js [fixture.rgba] [width] [height]
//   default: scratch/sample512.rgba 512 384
//
...
const fixture = process.argv[2] || path.join(ROOT, 'scratch', 'sample512.rgba');
const W = Number(process.argv[3] || 512);
const H = Number(process.argv[4] || 384);
...
const EXPECT_PEAKS = Number(process.env.EXPECT_PEAKS || 101);
const PEAK_TOL = Number(process.env.PEAK_TOL || 5);
const EXPECT_MAX = Number(process.env.EXPECT_MAX || 0.6479);
const MAX_TOL = 0.01;
...
(async () => {
  ...
  if (res.map_w !== W / DOWN || res.map_h !== H / DOWN) {
    fail('map is ' + res.map_w + 'x' + res.map_h + ', expected ' + (W / DOWN) + 'x' + (H / DOWN));
  }
